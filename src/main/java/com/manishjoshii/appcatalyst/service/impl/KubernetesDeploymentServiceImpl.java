...
public class KubernetesDeploymentServiceImpl implements DeploymentService {
    ...
    private static final String NAMESPACE = "acs-apps";
    ...
    private DeployResponse claimAndStartNewPod(Long projectId, String domain) {
        ...
        try {
            ...
            log.info("Pod {} claimed successfully for project {}", claimedPod.getMetadata().getName(), projectId);
        } catch (Exception e) {
            log.error("Failed to claim pod {} for project {}", podName, projectId, e);
            throw new RuntimeException("Could not claim an idle runner", e);
        }
        ...
        try {
            ...
            // Runner Commands (npm install && nohangup num run dev on host 0.0.0.0:5173 save logs to /app/dev.log)
    ...
    private void registerRoute(String domain, Pod pod) {
        ...
        redisTemplate.opsForValue().set("route:" + domain, podIp + ":5173", Duration.ofHours(6));
    }
